...
// ─── Nav keys ────────────────────────────────────────────────────────────────
type NavKey = "review" | "marketplace" | "automation" | "reports" | "activity";

const sideNav: { key: NavKey; label: string; helper: string; icon: React.ElementType }[] = [
  { key: "review", label: "Review Mode", helper: "Applications & approvals", icon: FileSearch },
  { key: "marketplace", label: "Marketplace Mode", helper: "Visibility & ranking", icon: BadgeCheck },
  { key: "automation", label: "Automation Mode", helper: "Emails & follow-ups", icon: Mail },
  { key: "reports", label: "Reports", helper: "Leads, match & performance", icon: BarChart3 },
  { key: "activity", label: "Activity Log", helper: "Audit trail", icon: Activity },
];
...
const applicationCards = [
  { name: "Ana Khan", role: "Principal Security Architect", firm: "Sentinel Forge", specialty: "Network Security", submitted: "2 hours ago", profileCompletion: 96, credentials: "ID verified · 3 certifications uploaded", compliance: "Business license attached · Chamber letter included", status: "Under Review" as AppStatus, activityHistory: ["Submitted 2 hours ago", "Auto-check passed"] },
  { name: "Daniel Cho", role: "Incident Response Lead", firm: "BlueTrace Labs", specialty: "Endpoint Security", submitted: "5 hours ago", profileCompletion: 72, credentials: "Identity check complete · Insurance pending", compliance: "License review incomplete", status: "Needs Information" as AppStatus, activityHistory: ["Submitted 5 hours ago", "Info requested by Admin on Apr 7"] },
  { name: "Sofia Ramirez", role: "IAM Program Consultant", firm: "AccessGrid Advisory", specialty: "Cloud Security", submitted: "1 day ago", profileCompletion: 94, credentials: "Identity verified · 4 certifications uploaded", compliance: "License complete · References approved", status: "Ready to Approve" as AppStatus, activityHistory: ["Submitted 1 day ago", "All docs verified", "Flagged as Ready by Admin on Apr 6"] },
];

const marketplaceExperts = [
  { name: "Ana Khan", specialties: "Operations, Growth, Legal Help", location: "Nassau", verified: true, featured: true, visible: true, rankingWeight: "High" as WeightLevel, leadTier: "Premium" as const, response: "Fast Response" },
  { name: "Daniel Cho", specialties: "Cybersecurity, Systems, Personal Tech", location: "Grand Bahama", verified: true, featured: false, visible: false, rankingWeight: "Medium" as WeightLevel, leadTier: "Premium" as const, response: "This Week" },
  { name: "Sofia Ramirez", specialties: "Financial, Career, Operations", location: "Remote / Nassau", verified: false, featured: true, visible: true, rankingWeight: "Low" as WeightLevel, leadTier: "Standard" as const, response: "Fast Response" },
];

const matchingCriteria = [
  { factor: "Expertise Match", weight: "High" as WeightLevel, description: "How closely the expert's specialty aligns with the client's problem" },
  { factor: "Urgency Alignment", weight: "High" as WeightLevel, description: "Whether the expert can respond within the client's required timeframe" },
  { factor: "Location / Proximity", weight: "Medium" as WeightLevel, description: "Same island or remote-friendly preference" },
  { factor: "Availability", weight: "Medium" as WeightLevel, description: "Current capacity and preferred work schedule" },
  { factor: "Prior Experience", weight: "Low" as WeightLevel, description: "Client's past consulting experience affecting match sensitivity" },
];
...
function ReviewMode() {
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-[28px] border border-[#FED7A0] bg-[#FFFBF0] p-5">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        {[
          ...
          <article key={s.label} className="rounded-[24px] border border-[#D9E3F3] bg-white p-5 shadow-[0_10px_24px_rgba(56,75,107,0.06)]">
            <p className="text-xs uppercase tracking-[0.12em] text-[#7B89A2]">{s.label}</p>
      ...
      {/* Review Queue */}
      <div className="rounded-[30px] border border-[#D9E3F3] bg-white p-5 shadow-[0_14px_34px_rgba(56,75,107,0.08)]">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <div>
          ...
          <div className="overflow-hidden rounded-[20px] border border-[#D9E3F3]">
            ...
            <div className="flex items-center justify-between border-t border-[#EEF2FA] px-4 py-3">
...
export default function AdminPage() {
  const [activeKey, setActiveKey] = useState<NavKey>("review");
  ...
  return (
    <main className="sl-page min-h-screen text-[#111827]">
      <div className="mx-auto grid w-full max-w-[1420px] gap-6 px-6 py-8 lg:grid-cols-[280px_1fr]">
        ...
        {/* ── Sidebar ── */}
        <aside className="self-start rounded-[30px] border border-[#D9E3F3] bg-white p-5 shadow-[0_14px_34px_rgba(56,75,107,0.08)] lg:sticky lg:top-8">
          <div className="rounded-[24px] border border-[#D9E3F3] bg-[linear-gradient(180deg,#ffffff_0%,#f5f9ff_100%)] px-4 py-4">
            ...
            <p className="mt-1 text-sm leading-6 text-[#5D6B85]">Expert onboarding, marketplace, automation, and reporting.</p>
          </div>
            ...
            {sideNav.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-2xl border px-4 py-3 text-left transition ${isActive ? "border-[#BFD0F8] bg-[#EEF3FF]" : "border-transparent bg-transparent hover:border-[#D9E3F3] hover:bg-[#F7FAFF]"}`}
                >
                  <span className="inline-flex items-start gap-3">
                    <span className={`mt-0.5 inline-flex h-9 w-9 items-center justify-center rounded-xl ${isActive ? "bg-white text-[#356AF6]" : "bg-[#F4F7FC] text-[#7B89A2]"}`}>
                      <Icon className="h-4 w-4" />
                    </span>
                    <span>
                      <span className={`block text-sm font-semibold ${isActive ? "text-[#356AF6]" : "text-[#111827]"}`}>{item.label}</span>
                      <span className="mt-1 block text-xs text-[#7B89A2]">{item.helper}</span>
                    </span>
                  </span>
                  <ChevronRight className={`h-4 w-4 transition ${isActive ? "text-[#356AF6]" : "text-[#A0AECB]"}`} />
                </button>
              );
            })}
          </nav>

          {/* Quick Actions — only shown in review mode where they're relevant */}
          {activeKey === "review" && (
            <div className="mt-6 rounded-[24px] border border-[#D9E3F3] bg-[#FCFDFF] p-4">
              ...
              <div className="mt-3 space-y-2">
                ...
                <Button asChild className="flex w-full items-center gap-2 rounded-xl border border-[#D9E3F3] bg-white px-3 py-2.5 h-auto text-xs font-semibold text-[#111827] shadow-none hover:bg-[#F7FAFF]">
        ...
        <section>
          <div className="mb-6">
            <InnerNav breadcrumb={pageTitles[activeKey]} />
          </div>

          {activeKey === "review" && <ReviewMode />}
          {activeKey === "marketplace" && <MarketplaceMode />}
          {activeKey === "automation" && <AutomationMode />}
          {activeKey === "reports" && <ReportsView />}
          {activeKey === "activity" && <ActivityLogView />}
        </section>
